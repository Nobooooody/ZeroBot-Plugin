...
func renderForwardMsg(qq int64, raw string) (base64Bytes []byte, err error) {
	...
	if qq != 0 {
		...
		face, err = web.RequestDataWith(web.NewDefaultClient(), fmt.Sprintf(anonymousURL, rand.Intn(4)+1), "GET", "gitcode.net", web.RandUA(), nil)
	}
	if err != nil {
		return
	}
	...
	back := factory.Size(faceImg, backX, backY).Circle(0).Image()
	m := message.ParseMessageFromString(raw)
	maxHeight += margin
	...
	for _, v := range m {
		...
		if msgImg.Bounds().Dx() > 500 {
			msgImg = factory.Size(msgImg, 500, msgImg.Bounds().Dy()*500/msgImg.Bounds().Dx()).Image()
		}
